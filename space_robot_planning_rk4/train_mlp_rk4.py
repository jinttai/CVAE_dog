...
def plot_trajectory(q_traj, q_dot_traj, epoch):
    ...
    axes[0].set_title(f"[RK4] MLP Joint Angles (Epoch {epoch})")
    ...
    axes[1].set_title("[RK4] MLP Joint Velocities")
...
def main():
    ...
    print(f"=== MLP Training (Quaternion RK4 Physics) Start on {device} ===")

    robot, _ = urdf2robot("assets/SC_ur10e.urdf", verbose_flag=False, device=device)

    writer = SummaryWriter(log_dir="runs/mlp_rk4_v1")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        print(f"[RK4] MLP Epoch [{epoch+1}/{NUM_EPOCHS}] | Loss: {loss_value:.6f} | Time: {epoch_duration:.2f}s")
        epoch_start_time = time.time()
    ...
    if len(train_losses) > 0:
        ...
        csv_dir = os.path.join(plots_dir, "mlp_training_curve_rk4")
        ...
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.title("MLP Training Curve (Quaternion RK4 Physics)")
        plt.grid(True)
        plt.legend()
        save_dir = os.path.join(plots_dir, "mlp_training_curve_rk4")
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, "v1.png")
        plt.savefig(save_path, dpi=150, bbox_inches="tight")
        plt.close()

    save_dir = os.path.join("weights", "mlp_rk4_debug")
    os.makedirs(save_dir, exist_ok=True)
